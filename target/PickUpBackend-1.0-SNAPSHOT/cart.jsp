...
<head>
    ...
    function validarCantidad(input) {
        if (input.value < 1) {
            ...
            input.value = 1; // Restablecer el valor
        }
    }
</script>
...
<div class="dropdown my-auto">
    ...
    <div class="container mt-5">
                ...
                <tbody>
                    ...
                    <c:if test="${not empty sessionScope.carrito}">
                        ...
                        <c:forEach var="item" items="${sessionScope.carrito}">
                            <tr>
                                <td><img src="${item.producto.imagen}" alt="${item.producto.nombre}" style="width: 80px; height: 80px;" class="rounded-circle"></td>
                                <td>${item.producto.nombre}</td>
                                <td>S/ ${item.precioUnitario}</td>
                                <td>
                                    <form action="${pageContext.request.contextPath}/ProductoServlet" method="get" style="display: inline;">
                                        <input type="hidden" name="accion" value="actualizarCantidad">
                                        <input type="hidden" name="productoId" value="${item.idProducto}">
                                        <input type="number" name="cantidad" value="${item.cantidad}" min="1" style="width: 60px;" oninput="validarCantidad(this)">
                            ...
                            <!-- Calcular subtotal -->
                            <c:set var="subtotal" value="${subtotal + (item.cantidad * item.precioUnitario)}" />
                        </c:forEach>

                        <!-- Guardar el subtotal en la sesión -->
                        <% session.setAttribute("subtotal", pageContext.findAttribute("subtotal")); %>
                    </c:if>
                    ...
                    <c:if test="${empty sessionScope.carrito}">
                        <tr>
                            <td colspan="6" class="text-center">
                                ...
                                <a href="${pageContext.request.contextPath}/shop.jsp" class="btn btn-primary">Volver al Catálogo</a>
                            </td>
                        </tr>
                    </c:if>
                </tbody>
        ...
        <!-- Resumen de costos -->
<div class="mt-4">
    <h3 class="text-end">Resumen</h3>
    <p class="text-end">Subtotal: <strong>S/ ${subtotal}</strong></p>
    <p class="text-end">Total: <strong>S/ ${subtotal}</strong></p> <!-- El total es igual al subtotal aquí -->
    <div class="text-end">
        <!-- Mostrar el botón si el carrito tiene productos -->
        <c:if test="${not empty sessionScope.carrito}">
            <form action="${pageContext.request.contextPath}/chackout.jsp" method="get">
                <button type="submit" class="btn btn-primary">Proceder con el Pago</button>
            </form>
        </c:if>
        <!-- Mensaje si el carrito está vacío -->
        <c:if test="${empty sessionScope.carrito}">
            <p class="text-danger">El carrito está vacío. Agrega productos para continuar.</p>
        </c:if>
    </div>
</div>
